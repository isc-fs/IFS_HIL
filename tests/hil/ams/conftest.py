...
def current_heartbeat(ams_profile, mlc_powered):
    ...
    def _ma_to_volts(mA: int) -> float:
        ...
        #   kCurrentZeroMv     = 2500   (2.5 V at 0 A)
        #   kCurrentMvPerAmpe1 = 57     (firmware stores 10x mV/A, so 5.7 mV/A)
        # `inline constexpr std::int32_t kCurrentZeroMv = 2500;` /
        # `inline constexpr std::int32_t kCurrentMvPerAmpe1 = 57;` in
        # firmware/Core/Inc/app/ams_config.hpp. Re-verify here if firmware
        # COMMISSIONING values change.
...
# ---------------------------------------------------------------------------
# AMS-side relay-GPIO readback via MCP3208 ADC.
#
# AMS firmware drives PB4 (AMS_OK), PB5 (AIR+), PB6 (AIR-), PB7 (PRECH)
# as outputs to control contactors on a real car. On the bench they're
# sampled by ADC3 (MCP3208, broker idx 2) channels 0..3 so we can verify
# the FSM is actually toggling them as expected. Per the user's bench
# wiring spec.
# ---------------------------------------------------------------------------
