...
|-------|------------------------------------------------------------------|-------------|
| K-100 | 0x4A0 byte-by-byte decode matches stub seed across all 6 states  | implemented |
...
class TestK100Frame0x4A0DecodeAcrossStates:
    ...
    def test_k100(self, fresh_boot, tsms, dash_chg, acu_heartbeat,
        ...
        snapshot(M.FsmState.TRANSITION)
...
# ---------------------------------------------------------------------------
# The flight encoder leaves 0x4A2[5] at 0; the HIL_STUB encoder packs
# the cockpit byte there with bit 7 set as a sentinel. If a flight
# build sneaks onto the bench this test fails loudly -- you don't want
# to spend hours debugging "why doesn't the cockpit fixture do anything"
# only to find out the binary was compiled wrong.
...
class TestK102HilLayoutActive:
    ...
    def test_k102_cockpit_byte_sentinel_set(self, fresh_boot, observe_acu,
        ...
        assert cb["valid"], (
            ...
            f"Binary on the bench is probably a flight build -- rebuild "
            f"with `-DAMS_BMS_HIL_STUB=1` and reflash."
        )
